...
handle_info(check_file, State = #state{file = File, mtime = MTime}) ->
...
handle_info(check_cron, State = #state{entrys = Entrys}) ->
...
check_file_timer() ->
    erlang:send_after(?CHECK_FILE_INTERVAL, self(), check_file).

%% start the cron tasks timer
check_cron_timer() ->
    erlang:send_after(?CHECK_CRON_INTERVAL, self(), check_cron).
...
parse(File) ->
    case util:consult_nested(File) of
        {error, enoent} = Error ->
